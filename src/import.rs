...
pub async fn import(
    ...
    // Get column mapping
    let column_mapping = options.column_mapping.clone().unwrap_or_else(|| {
        csv_columns
            .iter()
    ...
    if !table_exists {
        ...
        println!("Table '{}' already exists, inserting data...", options.table);
    }
                ...
                match parse_row(&row, &csv_columns, &db_columns, &inferred_types) {
                    Ok(values) => {
                        ...
                        if batch.len() >= options.batch_rows {
                            session
                                .insert_batch(&options.table, &db_columns, &batch)
                                .await
...
fn infer_column_types(
    ...
    for scores in type_scores {
        let final_type = if let Some((type_name, _)) =
            scores.iter().max_by_key(|(_, &count)| count)
        {
